...
if __name__ == "__main__":
    ...
    indicators_df_postcode = (indicators_df
                                   .join(info_stations_df.select("station_id", "postcode"), ["station_id"], "left")
                                   .drop("station_id")
                                   )

    # Add "timestamp" column
    indicators_df_postcode = indicators_df_postcode.withColumn("timestamp", pysqlf.current_timestamp())

    # Set columns order
    col_order = ["timestamp", "postcode", "total_bikes_available", "total_mechanical_bikes", "total_electric_bikes"]
    indicators_df_postcode = indicators_df_postcode.select(*col_order)
    
    # Display results in Python output
    query_console = (indicators_df_postcode
            .writeStream
            .outputMode("complete")
            .format("console")
            .start()
    ...
    query_kafka = (indicators_df_postcode
            .writeStream
            .outputMode("complete")
            .format("kafka")
            .option("kafka.bootstrap.servers", "localhost:9092")
